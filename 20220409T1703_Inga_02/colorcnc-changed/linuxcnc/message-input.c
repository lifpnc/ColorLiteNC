...
void firstRun(data_t *port)
{
	...
	*(uint32_t*)&wb_wr_buffer[20]=htobe32(csr_addr(CSR_MAGIC_OFFSET+4)); //data address
	...
	for ( int tryCount = 5*10, first=1; tryCount >= 0; --tryCount, first = 0)
	{
		...
		eb_send(port->eb, wb_wr_buffer, 24);
	    }
	    
	    int rcMagic = eb_recv(port->eb, wb_rd_buffer, sizeof(wb_rd_buffer));
		...
		LL_ERROR("******** cannot read magic from card. Terminating ***** \n");
		hal_exit(comp_id);
	    }
	    ...
	    if (!check_magic(wb_rd_buffer, 16, 1))  // offset 16: First payload word
	    {
		    ...
		    LL_ERROR("There is no magic in this card, got %d, expected %d, exit!\n", wb_rd_buffer[16], MAGIC_CNC);
		    hal_exit(comp_id);
	    }
	    break;
	}
...
dump_buffer(wb_wr_buffer, pos1, "colorcnc debug: request input data", 0);
    ...
    // #################  TRANSMIT DATA  ###############################################################
        int res=eb_send(port->eb, wb_wr_buffer, pos1);
	if ( res < 0) {
...
long tsr1 = timestampMsec();
        ...
        int count = eb_recv(port->eb, wb_rd_buffer, sizeof(wb_rd_buffer));
/*debug*/
long tsr2 = timestampMsec();
	    ...
	    if ( hErrorCountTotal > MAX_ERROR_TOTAL || hErrorCountCont > MAX_ERROR_CONT ) {
            ...
            LL_ERROR("update_port: Connection broken after %ld sec, errorCount %ld ***, try again\n", (hLastReceiveTime - hStartTime), hErrorCountTotal );
	    return 0;
        }
	else if ( count != expectedLength) // request and reply packet have the same size
	{
	    LL_ERROR("update port: requested port data packet size mismatch %d / %d (ignored)\n", count, expectedLength);
	    return 0; // ignore
	}
...
/* debug */
//LL_DEBUG("recv %d bytes reply after %ld msec\n", count, ( tsr2-tsr1));
dump_buffer(wb_rd_buffer, count, "received input data", 0);
...
if (tmpwdc < 100000) {
	...
	//dump_buffer(wb_rd_buffer, count, "colorcnc debug: received input data (watchdog low)", 1);
}
//debug
///uint64_t tmpdebug=htobe64(*(__u64*)&wb_rd_buffer[16+6*8+8+4+4]);
//	if (tmpdebug != 0) LL_DEBUG("debug data received: %ld\n", tmpdebug);
	...
	discard_pending_packet(port->eb);
