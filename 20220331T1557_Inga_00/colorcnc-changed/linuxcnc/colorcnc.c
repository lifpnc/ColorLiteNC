...
//#define DELAY 50  // usec delay between two send packets
...
void update_port(void *arg, long period){
	...
	// Watchdog first
        ...
        for (i=0;i<num_records_to_write-1;i++)
        {
...
/*
	// debug data (64 bit)
        tmp_data=htobe32(csr_addr(CSR_AFTER_READ)); //fill addresses to read
        memcpy((void*)&wb_wr_buffer[pos1],&tmp_data,4);
	pos1 += 4;
        tmp_data=htobe32(csr_addr(CSR_AFTER_READ+4)); //fill addresses to read
        memcpy((void*)&wb_wr_buffer[pos1],&tmp_data,4);
	pos1 += 4;
*/
	...
	int requestSize = pos1;

/* debug */
dump_buffer(wb_wr_buffer, requestSize, "colorcnc debug: request input data", 0);

    // #################  TRANSMIT DATA  ###############################################################
        res=eb_send(device_data->eb, wb_wr_buffer, requestSize);
...
long tsr2 = timestampMsec();
	    ...
	    ++hErrorCount;
	    if ( hErrorCount > 50 ) {
		    fprintf(stderr, "\ncolorcnc: too many receive error. exit.\n\n");
		    exit(1);
	    }
            //fprintf(stderr, "colorcnc update_port: unexpected read length: %d after %ld msec\r", count, (tsr2-tsr1));
            fprintf(stderr, "colorcnc update_port: Connection broken after %ld sec, errorCount %ld ***\r", (hLastReceiveTime - hStartTime), hErrorCount );
            return;
        }
	else if ( count != requestSize) // request and reply packet have the same size
	{
		fprintf(stderr, "colorcnc update port: requested port data packet size mismatch %d / %d (ignored)\n", count, requestSize);
		return; // ignore
	}
...
if (tmpwdc < 100000) {
	fprintf(stderr, "colorcnc: Watchdog received: %d\n", tmpwdc);
...
int check_magic( unsigned char * buffer, int offset, int complain)
{
    ...
    if (version != MAGIC_CNC) {
	    if (complain) fprintf(stderr, "\n\nERROR: colorcnc: the is no magic in this card, got %d, expected %d, exit!\n", version, MAGIC_CNC);
	    return 0;
    }
    int inputs = buffer[offset+1];
    if (inputs != di_count) {
	    if (complain) fprintf(stderr, "\n\nERROR: colorcnc: input count mismatch, got %d, expected %d, exit!\n", inputs, di_count);
	    return 0;
    }
    int outputs = buffer[offset+2];
    if (outputs != do_count) {
	    if (complain) fprintf(stderr, "\n\nERROR: colorcnc: output count mismatch, got %d, expected %d, exit!\n", outputs, do_count);
	    return 0;
    }
    int steppercount = buffer[offset+3];
    if (steppercount != steppers_count) {
	    if (complain) fprintf(stderr, "\n\nERROR: colorcnc: steppers count mismatch, got %d, expected %d, exit!\n", steppercount, steppers_count);
	    return 0;
    }
    int pwmcount = buffer[offset+4];
    if (pwmcount != pwm_count) {
	    if (complain) fprintf(stderr, "\n\nERROR: colorcnc: pwm count mismatch, got %d, expected %d, exit!\n", pwmcount, pwm_count);
	    return 0;
    }
