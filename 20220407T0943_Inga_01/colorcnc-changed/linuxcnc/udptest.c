...
//fprintf(stderr, "udptest debug: enter udp_raw_send\n");
	...
	{
		fprintf(stderr,"udptest: payload data length %d exceeds maximum lenght\n", payloadLen);
		exit(1);
	}
...
//fprintf(stderr, "udptest debug: iph done\n");
	
	//Ip checksum
	iph->check = csum ((unsigned short *) datagram, iph->tot_len);

//fprintf(stderr, "udptest debug: csum done\n");
...
//fprintf(stderr, "udptest debug: udph done\n");
...
//fprintf(stderr, "udptest debug: psh done\n");
...
//fprintf(stderr, "udptest debug: checksum done\n");
	
	//loop if you want to flood :)
	//while (1)
	{
		
//fprintf(stderr, "udp raw socket: %d, adr = %d, port = %d\n", conn->fd, raw_sin.sin_addr, raw_sin.sin_port);
//dump_buffer(datagram, iph->tot_len, "udp raw data",1);
		...
		{
			//printf ("udptest: Packet Send. Length : %d \n" , iph->tot_len);
		}
	}
//fprintf(stderr, "udptest debug: exit udp_raw_send\n");
