...
************************************************************************/
static char *board_ip="192.168.2.50";
...
static uint32_t num_errors_reported;
...
/* Extracts from the etherbone network packet the bytes in 16 to 19 position and returns them as an int
 * in case of a received packet, it has the structure of a write packet received from the board, and contains
 * data in bytes 16 to 19*/
...
int eb_fill_readwrite32(uint8_t wb_buffer[20], uint32_t data, uint32_t address, int is_read) {
	...
	// one record per packet limit in litex
    if (is_read) {
        wb_buffer[10] = 0;  // Write count
        wb_buffer[11] = 1;	// Read count
        data = htobe32(address);
        memcpy(&wb_buffer[16], &data, sizeof(data)); // in case of read byte 16 to 19 read address
    }	// one record per packet limit in litex
    else {
        wb_buffer[10] = 1;	// Write count
        wb_buffer[11] = 0;  // Read count
        address = htobe32(address);
        memcpy(&wb_buffer[12], &address, sizeof(address));// in case of write byte 12 to 15 write address

        data = htobe32(data);
        memcpy(&wb_buffer[16], &data, sizeof(data));// in case of write byte 16 to 19 value to write
    }
    return 20;
}

/* creates an etherbone write packet for writing data to address*/
...
/* creates an etherbone write packet for reading data from address*/
...
uint32_t eb_read32(struct eb_connection *conn, uint32_t addr) {
    ...
    if (count != sizeof(raw_pkt)) {
        fprintf(stderr, "eb_read32 unexpected read length: %d\n", count);
        return -1;
    }
...
//###################################################
    device_data->eb = eb_connect("192.168.2.50", "1234", 1);
...
/* export encoder signal */
	...
	for ( i=0; i<encoders_count; i++) {
        ...
        // encoder_scale
		retval = hal_param_float_newf(HAL_RW, &(device_data-> encscale[i]), comp_id, "colorcnc.%d.feedback.enc_scale%d", 1, i);
		if (retval < 0) {
...
/////////////////////////////
	...
	for ( i=0; i<steppers_count;i++) {
       ...
       r = hal_pin_float_newf(HAL_IN, &(device_data->stepgen_velocity_cmd[i]), comp_id, "colorcnc.%d.stepgen.%01d.velocity-cmd", 1, i);
        ...
        r = hal_param_bit_newf(HAL_RW, &(device_data->stepgen_inverse_step[i]), comp_id, "colorcnc.%d.stepgen.%01d.inverse_step", 1, i);
        if(r != 0) return r;
        device_data->stepgen_inverse_step[i] = 0;

        r = hal_param_bit_newf(HAL_RW, &(device_data->stepgen_inverse_dir[i]), comp_id, "colorcnc.%d.stepgen.%01d.inverse_dir", 1, i);
        if(r != 0) return r;
        device_data->stepgen_inverse_dir[i] = 0;
...
////////////////////////////////////////
	...
	*(uint32_t*)&wb_wr_buffer[20]=htobe32(0x88+4); //data address
	// #################  TRANSMIT DATA  ###############################################################
	r=eb_send(device_data->eb, wb_wr_buffer, 24);
	if (r>0)
	{
		int count = eb_recv(device_data->eb, wb_rd_buffer, sizeof(wb_rd_buffer));
		if (count==24)
		{
			device_data->board_wallclock=htobe64(*(__u64*)&wb_rd_buffer[16]);
			rtapi_print_msg(RTAPI_MSG_ERR,"colorcnc: INFO: colorcnc timestamp %llu\n", device_data->board_wallclock);
		}
	}
...
#undef enccounts
#define enccounts(i) (*(port->enccounts[i]))
#undef encscale
#define encscale(i) (port->encscale[i])
/**************************************************************
###############################################################
...
void update_port(void *arg, long period){
	...
	pack_mainW_t send_payload;
      ...
      /*  if(fmax > COLORCNC_FREQ / (2<<speedrange))
        ...
        */
        ...
        send_payload.steppers_ctrlword|=port->stepgen_inverse_step[i]?1<<(i*2+10):0;        
        send_payload.steppers_ctrlword|=port->stepgen_inverse_dir[i]?1<<(i*2+11):0;        
        ...
        if(port->first_run)
		{
			send_payload.steppers_ctrlword|=2; //reset signal
		}
		port->first_run=0;
        ...
        num_records_to_read=21;//(sizeof(pack_mainR_t))/sizeof(__u32); // mod
    ...
    // #################################################################################################
        ...
        {
            tmp_data=htobe32(0x60+i*4); //from first address to read, fill addresses to read
    ...
    // #################  RECEIVE DATA  ###############################################################
		...
		{
			fprintf(stderr, "eb_recv connection restored\n", count);
			num_errors_reported=0;
			}
                tmp1=htobe32(*(__u32*)&wb_rd_buffer[16+6*8+8]); // 72
	...
	port->board_wallclock=htobe64(*(__u64*)&wb_rd_buffer[16+6*8]);//htobe64(t->board_wallclock); 64
	*(port->board_wallclock_msb)=htobe32(*(__u32*)&wb_rd_buffer[16+6*8]); // 64
	*(port->board_wallclock_lsb)=htobe32(*(__u32*)&wb_rd_buffer[16+6*8+4]); // 68
    ...
    ///////////// read s/d count /////////////////////////////////////////////
            ...
            for ( i=0; i<steppers_count;i++)
            {
                ...
                newlow = (htobe64(*(__u64*)&wb_rd_buffer[i*8+16]));// & MASK; 16..64
     ...
     ///////////// read encoders /////////////////////////////////////////////
            ...
            for ( i=0; i<encoders_count;i++)
            {
                enccounts(i) = ((hal_float_t)((int32_t)(htobe32(*(__u32*)&wb_rd_buffer[16+6*8+8+4+i*4]))))*encscale(i); // 76..100;
            }
